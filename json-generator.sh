...
do
    ...
    then
        ...
        do
            if [ $j -ne $((END-1)) ]
            then
                echo -e "\t\t\"$j\":${x_array[$j]}," >> result.json
            else
                echo -e "\t\t\"$j\":${x_array[$j]}" >> result.json
            fi
            ((j++))   
    ...
    then
        ...
        do
            if [ $j -ne $((END-1)) ]
            then
                echo -e "\t\t\"$j\":${y_array[$j]}," >> result.json
            else
                echo -e "\t\t\"$j\":${y_array[$j]}" >> result.json
            fi
            ((j++))
    ...
    then
        ...
        do
            if [ $j -ne $((END-1)) ]
            then
                echo -e "\t\t\"$j\":${z_array[$j]}," >> result.json
            else
                echo -e "\t\t\"$j\":${z_array[$j]}" >> result.json
            fi
            ((j++))
...
chown luislavieri:sudo result.json

chmod g+rw result.json
